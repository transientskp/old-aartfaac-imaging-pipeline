...
class gpuTextLogHdlr(logHdlr):
	...
	def __init__ (self, spec):
		...
		self._knownkeys = {'TOBS'          : numpy.empty ( (NRec2Buf, 1) ), 
						   'FLAGPERCENT'   : numpy.empty ( (NRec2Buf, Nelem) ), 
						   'EXECTIME'      : numpy.empty ( (NRec2Buf, 1) ),
						   'LATETIME'      : numpy.empty ( (NRec2Buf, 1) )};
	...
	def readRec (self):
		...
		while not recdone:
			...
			if 'stats' in self._line:
				...
				if m:
					self._knownkeys['TOBS'][self._recnum] = numpy.uint32(m.group(1));
					...
					# if times[i] == 0:
					#    times[i] = time
					self._knownkeys['FLAGPERCENT'][self._recnum,station] = flag;
				...
				if m:
					self._knownkeys['EXECTIME'][self._recnum,0] = numpy.float(m.group(3));
					self._knownkeys['LATETIME'][self._recnum,0] = numpy.float(m.group(6));
					# print 'Match: recnum: %d, late: %f, exec: %f ' % (self._recnum, numpy.float(m.group(3)), numpy.float(m.group(6)));

				# else:
				#     assert(times[i] == time)
		
class gpuSubbandHandler(logHdlr):
...
class pipelineMonTextHdlr(logHdlr):
...
if __name__ == '__main__':
	...
	o.add_option('-v', '--gpuvis=gpucorrip:port', dest='gpuvissrc', default='10.144.6.14:4099', help='Specify the source of GPU visibility stream, single subband. Uncalibrated images are created out of this.'); 
	
	o.add_option('-p', '--pipe=pipelineip:port', dest='pipe', default='ads001.control.lofar:4100', help='Specify the source of pipeline monitoring datastream.');
	o.add_option('-g', '--gpulog=gpucorrip:port', dest='gpulogsrc', default='gpu02.:4100', help='Specify the source of GPU logging datastream.');
	opts, args = o.parse_args(sys.argv[1:]);


	gpulog = gpuTextLogHdlr (opts.gpulogsrc);
	while (DoneRead == 0):
		gpulog.readRec();
"""
	pipehdlr = pipelineMonTextHdlr (opts.pipe);
	while (DoneRead == 0):
		pipehdlr.readRec();
