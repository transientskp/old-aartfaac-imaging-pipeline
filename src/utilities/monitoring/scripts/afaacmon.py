...
class pipelineMonTextHdlr:
	...
	# List of known keys 
	_knownkeys = {};
	_knownkeys = {'TOBS'            : numpy.empty ( (NRec2Buf, 1) ), 
				  ...
				  'GAINS'           : numpy.empty ( (NRec2Buf ,288), dtype=complex ),
	...
	def __init__ (self, desc):
		...
		if self._streamtype == 'file':
			print '--> Operating on file ', desc.split(':')[1];
			try:
				self._fid = open (desc.split(':')[1], "rt");	
			except IOError:
				print 'File ', desc.split(':')[1], ' does not exist. Quitting.';
				
		# Create a socket to connect to the specified address
		elif self._streamtype == 'tcp':
		...
		# Check what data is incoming, print the timestamp
		if (self._streamtype == 'file'):
...
if __name__ == '__main__':
	...
	o.add_option('-g', '--gpu=gpucorrip:port', dest='gpucorraddr', default='10.144.6.14:4099', help='Specify the source of GPU correlator stdout. This is parsed to generate logging plots.'); 
	...
	ax.plot (pipehdlr._knownkeys['TOBS'][0:pipehdlr._recnum,0], pipehdlr._knownkeys['FNORM'][0:pipehdlr._recnum,0], '-o');
