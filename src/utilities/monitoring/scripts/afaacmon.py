...
class logHdlr:
	...
	def __init__ (self, desc):
		...
		if self._streamtype == 'file':
			...
			print '### Unknown descriptor type ', self._streamtype;
			print '### Perhaps a descriptor (file:,tcp:) is missing? Quitting!';
			sys.exit();
...
class gpucorrVisHdlr(logHdlr):
	...
	def __init__ (self, desc, sbnum):
		...
		{'TOBS'			:{'DESC':'Time','UNIT':'Secs',
						  'VAL':numpy.empty ( (NRec2Buf, 1) )*numpy.nan},
		# 'VIS'			:{'DESC':'Complex visibility', 'UNIT':'ARBIT',
		#				  'VAL':numpy.zeros ([Nbline, Nchan, Npol, 2], 'f')},
		 #'INTEG_VIS'	:{'DESC':'Channel Integrated Complex visibility', 'UNIT':'ARBIT',
		#				  'VAL':numpy.zeros ([Nbline, Npol, 2], 'f')},
		 # 'ACM'		:{'DESC':'Complex ACM', 'UNIT':'ARBIT',
		 #				  'VAL':numpy.zeros ([Npol, Nelem*Nelem], dtype=complex)},
		 'ACM'			:{'DESC':'Complex ACM', 'UNIT':'ARBIT',
						  'VAL':numpy.zeros ([Npol, Nelem, Nelem], dtype=complex)},
		};
	...
	def readRec (self): # Can include number of records to read here TODO
		...
		self._knownkeywords['ACM']['VAL'] = self._acm_resh;
...
if __name__ == '__main__':
	...
	# Handle 
	if opts.gpuvissrc != 0:
		gpuvishdl = gpucorrVisHdlr (opts.gpuvissrc);

		# Subplot showing uncalibrated ACM ampl. 
		gpuvisampplt = subpltType ('Secs', 'Power', 
				'Subband ACM amplitude', 
				gpuvishdl._knownkeys['TOBS']['VAL'], 
				[gpuvishdl._knownkeys['ACM']['VAL']]); 
				
		# Subplot showing uncalibrated ACM phase. 
		gpuvisphplt = subpltType ('Secs', 'Deg', 
				'Subband ACM phase', 
				gpuvishdl._knownkeys['TOBS']['VAL'], 
				[gpuvishdl._knownkeys['ACM']['VAL']]); 
		logsrc.append(gpuvishdl);
		subplt.append(gpuvisampplt);
		subplt.append(gpuvisphplt);
...
"""
	# Main loop using sequential calls to readRec () of logHdlr objects.
	while (DoneRead == 0):
