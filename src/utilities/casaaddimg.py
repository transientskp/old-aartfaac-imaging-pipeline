...
os.system ('mkdir comb_subbands/');
for ind in range (1, min (len(sb0), len(sb1))):
	m = re.search ('_(.+?).image', sb1[ind]);
	outfile = 'comb_subbands/casacore'+m.group(1)+'.image';
	imagename = [sb0[ind], sb1[ind], sb2[ind], sb3[ind]]
	mode = 'evalexpr';
	expr = '(IM0 + IM1 + IM2 + IM3 )';
	print 'Combining ', sb0[ind], ' ', sb1[ind],' ', sb2[ind],' ', sb3[ind];
	immath ();
	print 'Done';
	print 'Exporting to fits'
	imagename = outfile;
	fitsimage = 'comb_subbands/5sb_'+m.group(1)+ '.image.fits';
	exportfits ();
	print 'Done';
