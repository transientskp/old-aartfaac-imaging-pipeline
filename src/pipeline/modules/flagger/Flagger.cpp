...
void Flagger::run(const StreamBlob *input, StreamBlob *output)
{
  for (int c = 0; c < input->mNumChannels; c++)
  {
    // Compute the amplitude of each complex value
    mAmplitudes = input->mData[c][XX_POL].cwiseAbs();
    ...
    for (int a = 0; a < NUM_ANTENNAS; a++)
    {
      if (mAntennas(a) < (median - threshold) || mAntennas(a) > (median + threshold))
      {
        ...
        output->mMasks[c][XX_POL].col(a).setOnes();
        output->mMasks[c][XX_POL].row(a).setOnes();
        output->mFlagged[c][XX_POL].push_back(a);
      }
    }
  }
}
