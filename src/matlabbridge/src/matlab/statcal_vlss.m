function [cal, sigmahat, Sigman] = statcal_vlss(acc, t_obs, freq, pos, srcsel, normal, restriction, maxrestriction, uvflag, catalog)
...
% Modified 22Mar12 by Peeyush for taking src catalog as an input.
...
for idx = 1:length(freq)
    ...
    if (sum(srcsel == 0) ~= 0)
        ...
        rasrc = catalog(srcsel).alpha;  % B1950
        decsrc = catalog(srcsel).delta; % B1950
